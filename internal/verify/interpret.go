...
func policyFindings(thresholds Thresholds, answers map[string]Answer) []Finding {
	...
	if conflictOK && conflict >= thresholds.ConflictMin {
		findings = append(findings, Finding{Code: "evidence_conflict", Verdict: VerdictConflict, Detail: "admissible evidence supports an incompatible conclusion"})
	}
	if supportOK && support < thresholds.SupportMin && !negative {
		findings = append(findings, Finding{Code: "support_below_threshold", Verdict: VerdictInsufficient, Detail: "support is below the policy threshold"})
	}
	if establishedOK && established < thresholds.EstablishMin {
		findings = append(findings, Finding{Code: "establishment_below_threshold", Verdict: VerdictInsufficient, Detail: "establishment is below the policy threshold"})
	}
	if negative {
		findings = append(findings, Finding{Code: "negative_result", Verdict: VerdictRejected, Detail: "admissible evidence establishes that the claim is not supported"})
	}
	if !actionOK {
		return findings
	}
	...
	case "manual_review", "other":
		findings = append(findings, Finding{Code: "review_required", Verdict: VerdictManualReview, Detail: "the action choice requires review"})
	case "proceed":
		if safetyOK && safety < thresholds.SafetyMin {
			findings = append(findings, Finding{Code: "safety_gate", Verdict: VerdictManualReview, Detail: "semantic safety is below the policy threshold"})
		}
		if (supportOK && support < thresholds.SupportMin) || (establishedOK && established < thresholds.EstablishMin) || (conflictOK && conflict >= thresholds.ConflictMin) {
			findings = append(findings, Finding{Code: "action_inconsistent", Verdict: VerdictManualReview, Detail: "the proceed choice disagrees with the predicate signals"})
		}
	case "reject":
		if supportOK && support >= thresholds.SupportMin {
			findings = append(findings, Finding{Code: "action_inconsistent", Verdict: VerdictManualReview, Detail: "the reject choice disagrees with support"})
		}
	}
	return findings
}
