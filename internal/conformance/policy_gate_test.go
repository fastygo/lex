...
func specGate(support, established, refuted, conflict, safety float64, action string) (string, []string) {
	...
	if established < thresholds.EstablishMin && !negative {
	...
	case "proceed":
		...
		if support < thresholds.SupportMin || established < thresholds.EstablishMin || conflict >= thresholds.ConflictMin || negative {
			codes = append(codes, "action_inconsistent")
		}
	case "reject":
		if !negative || established >= thresholds.EstablishMin {
			codes = append(codes, "action_inconsistent")
		}
	}
...
func TestIndependentPolicyGateAgreesWithVerifier(t *testing.T) {
	questions := map[string]verify.Question{
		...
		"refuted":          {Type: verify.QuestionNoul},
	...
	cases := []struct {
		name                                            string
		support, established, refuted, conflict, safety float64
		action                                          string
	}{
		...
		{name: "negative", support: 0.2, established: 0.1, refuted: 0.9, conflict: 0.1, safety: 0.9, action: "reject"},
...
func TestPolicyTruthTableAgreesWithVerifier(t *testing.T) {
	...
	seen := 0
	for _, refuted := range []float64{math.Nextafter(thresholds.RefuteMin, 0), thresholds.RefuteMin} {
				...
				for _, conflict := range conflicts {
					for _, safe := range safety {
						for _, action := range actions {
							seen++
							wantVerdict, wantCodes := specGate(support, establish, refuted, conflict, safe, action)
	...
	if seen != 128 {
		t.Fatalf("truth table cells = %d, want 128", seen)
	}
}
